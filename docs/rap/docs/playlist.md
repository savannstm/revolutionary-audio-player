...
By default, each view is sorted by path to the track. You can change sorting by clicking the required column.

View also allows you to drag and reorder tracks, and this order will be preserved. You can also sort tracks by their order.

It's possible to select multiple tracks for reordering/deletion by holding `Shift`/`Ctrl` when selecting the tracks.
...
Track-related options only show when you right-click a track in a tree, and tree-related options only show when you click outside any tracks.
